...
def most_successful(df, sport):
    ...
    # Get the top 15 most successful athletes and their counts
    top_athletes = temp_df['Name'].value_counts().head(15).reset_index()
    top_athletes.columns = ['Name', 'Count']  # Rename columns for clarity

    # Merge with specific columns from the original DataFrame
    merged_df = top_athletes.merge(df[['Name', 'Sport', 'region']], on='Name', how='left').drop_duplicates()
